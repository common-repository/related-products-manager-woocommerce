...
class ProWC_Related_Products_Manager_Settings_Per_Product {
	...
	function get_meta_box_options() {
		...
		$options = array(
			array(
				...
				'type'       => 'select',
				'options'    => array(
					'no'   => __( 'No', 'related-products-manager-woocommerce' ),
					'yes'  => __( 'Yes', 'related-products-manager-woocommerce' ),
					'hide' => __( 'Hide', 'related-products-manager-woocommerce' ),
				),
				'title'      => __( 'Enable', 'related-products-manager-woocommerce' ),
			),
			array(
				...
				'type'       => 'select',
				'options'    => $products,
				'title'      => __( 'Related products', 'related-products-manager-woocommerce' ),
				'multiple'   => true,
				'tooltip'    => ( $is_chosen_select ? ''              : __( 'Hold "Control" (Ctrl) key to select multiple values. "Control" and "A" to select all values.', 'related-products-manager-woocommerce' ) ),
			...
			array(
				...
				'title'      => __( 'Related categories', 'related-products-manager-woocommerce' ),
				'multiple'   => true,
				'tooltip'    => ( $is_chosen_select ? ''              : __( 'Hold "Control" (Ctrl) key to select multiple values. "Control" and "A" to select all values.', 'related-products-manager-woocommerce' ) ),
				'class'      => ( $is_chosen_select ? 'chosen_select' : '' ),
				'style'      => ( $is_chosen_select ? 'width:100%;'   : '' ),
			...
			array(
				...
				'title'      => __( 'Related tags', 'related-products-manager-woocommerce' ),
				'multiple'   => true,
				'tooltip'    => ( $is_chosen_select ? ''              : __( 'Hold "Control" (Ctrl) key to select multiple values. "Control" and "A" to select all values.', 'related-products-manager-woocommerce' ) ),
				'class'      => ( $is_chosen_select ? 'chosen_select' : '' ),
				'style'      => ( $is_chosen_select ? 'width:100%;'   : '' ),
	...
	function save_meta_box( $post_id, $post ) {
		...
		if ( ! isset( $_POST[ 'prowc_related_products_manager_' . $this->id . '_save_post' ] ) || ! check_admin_referer( 'prowc_related_products_manager_save_post_' . $this->id, 'prowc_related_products_manager_nonce' ) ) {
			return;
		}
	...
	function add_meta_box() {
		...
		add_meta_box(
			...
			__( 'Related Products', 'related-products-manager-woocommerce' ),
	...
	function create_meta_box() {
		...
		$html .= '</table>';
		$html .= wp_nonce_field('prowc_related_products_manager_save_post_' . $this->id, 'prowc_related_products_manager_nonce', true, false);
